...
public class FaceRecognition {
	...
	private INDArray forwardPass(INDArray indArray) {
		...
		INDArray embeddingValues = embeddings.doForward(false,
				LayerWorkspaceMgr.builder().defaultNoWorkspace().build());
...
//      System.out.println("encodingsValues =                 " + embeddingValues);
	...
	public String whoIs(String imagePath) throws IOException {
		...
		for (Map.Entry<String, INDArray> entry : memberEncodingsMap.entrySet()) {
			...
			System.out.println(
					"distance of " + entry.getKey() + " with " + new File(imagePath).getName() + " is " + distance);
	...
	public static void main(String[] args) throws Exception {
		FaceRecognition faceRecogntion = new FaceRecognition();
		faceRecogntion.loadModel();
